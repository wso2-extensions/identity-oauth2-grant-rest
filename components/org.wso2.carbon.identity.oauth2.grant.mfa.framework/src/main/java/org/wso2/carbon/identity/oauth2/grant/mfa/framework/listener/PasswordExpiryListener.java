...
public class PasswordExpiryListener extends AbstractAuthenticationListener {
	...
	public boolean doPreAuthenticate(AuthenticationContext authContext) throws MFAAuthException{
		...
		if (isUserPasswordExpired(tenantAwareUserName, authContext.getUser().getTenantDomain())){
			throw Util.handleClientException(
					Constants.ErrorMessage.CLIENT_EXPIRED_USER_PASSWORD, authContext.getUser().toFullQualifiedUsername());
		}
		return true;
	}

	@Override
	public boolean doPostAuthenticate(AuthenticationContext authContext) {
		return true;
	}
	 ...
	 * @throws MFAAuthException if the authentication failed for the user trying to login.
	 */
	public static boolean isUserPasswordExpired(String tenantAwareUsername, String tenantDomain)
		...
		try {
			...
			throw new MFAAuthException("Error occurred while loading user claim", e);
		}
	...
	private static Optional<String> getClaimValue(UserStoreManager userStoreManager,
												  String claimURI,
	 ...
	 * @throws MFAAuthException if retrieving property from resident idp fails.
	 */
	public static Optional<String> getResidentIdpProperty(String tenantDomain, String propertyName)
		...
		try {
			...
			throw new MFAAuthException("Error occurred while retrieving the resident IdP for tenant: " +
					tenantDomain, e);
		}
